...
public class EmployeeClient {
    ...
    public List<EmployeeDTO> getAllEmployees() {
        try {
                            ...
                            .retryWhen(
                                    Retry.backoff(3, Duration.ofMillis(200))
                                            ...
                                            .doBeforeRetry(retrySignal ->
                                                    log.warn("Retrying GET /employee due to 429 (attempt {} of 3). Cause={}",
            ...
            if (Objects.isNull(response)) {
                throw new ResponseStatusException(HttpStatus.BAD_GATEWAY, "Upstream returned empty response for getAllEmployees");
            }
            ...
            log.error("Upstream error during getAllEmployees. status={}, body={}",
                            ...
                            .retryWhen(
                                    Retry.backoff(3, Duration.ofMillis(200))
                                            ...
                                            .doBeforeRetry(retrySignal -> log.warn(
                                                    ...
                                                    id, retrySignal.totalRetries() + 1, retrySignal.failure().toString()
                                            ))
                            )
                            .block();

            if (Objects.isNull(response)) {
                throw new ResponseStatusException(HttpStatus.BAD_GATEWAY, "Upstream returned empty response for getEmployeeById");
            }

            log.debug("Upstream GET /employee/{} returned status='{}'", id, response.getStatus());

            if (Objects.isNull(response.getData())) {
                throw new ResponseStatusException(HttpStatus.BAD_GATEWAY, "Upstream returned null data for getEmployeeById");
            }

            return response.getData();

        } catch (WebClientResponseException ex) {
            log.error("Upstream error during getEmployeeById. id={}, status={}, body={}",
                    id, ex.getStatusCode(), ex.getResponseBodyAsString(), ex);
            throw ex;
        } catch (WebClientRequestException ex) {
    ...
    public EmployeeDTO createEmployee(CreateEmployeeRequest createEmployeeRequest) {
        try {
            ...
            if (Objects.isNull(response)) {
                throw new ResponseStatusException(HttpStatus.BAD_GATEWAY, "Upstream returned empty response for createEmployee");
            }

            log.debug("Upstream POST /employee returned status='{}'", response.getStatus());

            if (Objects.isNull(response.getData())) {
                throw new ResponseStatusException(HttpStatus.BAD_GATEWAY, "Upstream returned null data for createEmployee");
            }

            return response.getData();

        } catch (WebClientResponseException ex) {
            log.error("Upstream error during createEmployee. status={}, body={}",
                    ex.getStatusCode(), ex.getResponseBodyAsString(), ex);
            throw ex;
        } catch (WebClientRequestException ex) {
    ...
    public boolean deleteEmployeeByName(UpstreamDeleteEmployeeRequest upstreamDeleteEmployeeRequest) {
        try {
            UpstreamApiResponse<Boolean> response =
            ...
            if (Objects.isNull(response)) {
                throw new ResponseStatusException(HttpStatus.BAD_GATEWAY, "Upstream returned empty response for deleteEmployeeByName");
            }

            log.debug("Upstream DELETE /employee returned status='{}' for name='{}'",
                    response.getStatus(), upstreamDeleteEmployeeRequest.getName());

            if (Objects.isNull(response.getData())) {
                throw new ResponseStatusException(HttpStatus.BAD_GATEWAY, "Upstream returned null data for deleteEmployeeByName");
            }

            return response.getData();

        } catch (WebClientResponseException ex) {
            log.error("Upstream error during deleteEmployeeByName. name={}, status={}, body={}",
                    upstreamDeleteEmployeeRequest.getName(), ex.getStatusCode(), ex.getResponseBodyAsString(), ex);
            throw ex;
        } catch (WebClientRequestException ex) {
            log.error("Upstream request failed during deleteEmployeeByName. name={}",
                    upstreamDeleteEmployeeRequest.getName(), ex);
            throw ex;
        }
    }
}
